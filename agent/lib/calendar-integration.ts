...
const _CAL_COM_EVENT_TYPE = 'glamour-hair-salon';
...
export class CalComCalendar implements Calendar {
  ...
  async initialize(): Promise<void> {
    ...
    const meData = (await meResponse.json()) as { data: { username: string } };
    ...
    const eventTypesData = (await eventTypesResponse.json()) as { data: Array<{ slug: string; id: number }> };
    ...
    if (lkEventType) {
      ...
      const createResponse = await fetch(`${_BASE_URL}event-types`, {
        ...
        body: JSON.stringify({
          ...
          title: 'Glamour Hair Salon Appointment',
  ...
  async listAvailableSlots(options: { startTime: Date; endTime: Date }): Promise<AvailableSlot[]> {
    ...
    const rawData = (await response.json()) as { data: Record<string, Array<{ start: string }>> };
